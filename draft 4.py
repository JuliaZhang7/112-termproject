...
def distance(x1,y1,x2,y2):
    ydis=abs(y2-y1)
    xdis=abs(x2-x1)
    return math.sqrt(ydis**2+xdis**2)
...
def balltoBrickBounce(app,ball,brick):
    ...
    if ball.cx<brick.end[0]:
        ...
        if brick.slope<0:
            if (ball.mass*app.gravity*3)/(ball.hasbounced)>2:
                print('bouncing!!!')
                print('slope smaller than 0!')
                ball.dy=(-ball.mass*app.gravity*5)/(ball.hasbounced)
                print('dy',ball.dy)
    ...
    for ball in app.ballsreal:
        ...
        for brick in app.bricks:
            ...
            if checkInRange(ballxrange,brickXrange) and checkInRange(ballyrange,brickYrange):
                ...
                ball.forcex=0
                #ball.dx=0
def balltoBallMove(app,ball1,ball2):#ball1 and ball2 had collision,break to two
    normal=(ball2.cy-ball1.cy)/(ball2.cx-ball1.cx)
    #mball1*vballbefore+mball2*vballbefore=mball1*vballafter+mball2*vballafter
    print((ball1.mass*ball1.dy)/ball2.mass)
    ball1.cy-=(-(ball2.mass*ball2.dy)/ball1.mass)+3


def balltoBallColli(app):
    for firball in app.ballsreal:
        for secball in app.ballsreal:
            if firball==secball:
                pass
            else:
                if distance(firball.cx,firball.cy,secball.cx,secball.cy)<(firball.r+secball.r):
                    print('tada collision!!!!!')
                    balltoBallMove(app,firball,secball)
